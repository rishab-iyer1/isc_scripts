    ...
    with cProfile.Profile() as profile:
        if not os.path.exists(f"{sliding_perm_path}_{n_shifts}perms_{len(roi_selected)}rois"):
            with ProcessPoolExecutor() as executor:
                ...
                start = time()
                phase_slide_isc = list(tqdm(executor.map(phaseshift_sliding_isc, repeat(roi_selected[:2]), repeat(all_roi_masker),
...
x = {roi: [np.empty(shape=(n_windows, 1)), np.empty(shape=(n_windows, 1)), np.empty(shape=(n_shifts//batch_size, n_windows, 1))] for roi in roi_selected} # init empty dict with appropriate shapes
for roi in roi_selected:
    assert np.all(phase_slide_isc[0][roi][0] == phase_slide_isc[1][roi][0])  # make sure the "observed" is the same - should never change across batches
    assert np.all(phase_slide_isc[0][roi][0] == phase_slide_isc[2][roi][0])

    # joining the batched distributions
    dist = []
    for i in range(len(phase_slide_isc)):
        dist.append(phase_slide_isc[i][roi][2])

    x[roi][0] = phase_slide_isc[0][roi][0]
    x[roi][2] = np.concatenate(dist)  # concatenate all n_shifts permutations
    x[roi][1] = p_from_null(x[roi][0], x[roi][2], side='two-sided', exact=False, axis=0)
